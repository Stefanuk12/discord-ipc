...
pub fn unpack(data: Vec<u8>) -> Result<(u32, u32)> {
    ...
    let opcode = u32::from_le_bytes(opcode.try_into().map_err(|_| Error::MalformedOpcode)?);
    let header = u32::from_le_bytes(header.try_into().map_err(|_| Error::MalformedHeader)?);
